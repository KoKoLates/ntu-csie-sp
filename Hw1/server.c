...
int handle_id_data(request *reqP)
            ...
            { // select detects event
                ...
                {                                        // previous connection
                    ...
                    else if (ret == 0)
                    {
                        ...
                        { // inputed id; needs to release read lock
...
#ifdef READ_SERVER
                    // first check id
                    if (handle_id_data(&requestP[i]) != 1 && requestP[i].wait_for_write == 0)
                    { // invalid input
                    ...
                    {
                        // first handle lock
                        if (read_lock(file_fd, (off_t)requestP[i].id * sizeof(record)) < 0)
                        { // lock get by other process' write process
                            strcpy(buf, "Locked.\n");
                            write(requestP[i].conn_fd, buf, strlen(buf));
                            FD_CLR(requestP[i].conn_fd, &master_fds);
                            close(requestP[i].conn_fd);
                    ...
                    { // handle final output (upon receiving EXIT)
                    ...
                    if (requestP[i].wait_for_write == 0)
                    {
                        if (handle_id_data(&requestP[i]) != 1)
                        { // invalid input
                            strcpy(buf, "[Error] Operation failed. Please try again.\n");
                            write(requestP[i].conn_fd, buf, strlen(buf));
                            FD_CLR(requestP[i].conn_fd, &master_fds);
                        ...
                        else
                        {
                            ...
                            if (status < 0 || write_lock_ref_count[requestP[i].id] != 0)
                            { // already obtained lock in this process, or lock is occupied by other read
                                strcpy(buf, "Locked.\n");
                                write(requestP[i].conn_fd, buf, strlen(buf));
                                FD_CLR(requestP[i].conn_fd, &master_fds);
                                close(requestP[i].conn_fd);
